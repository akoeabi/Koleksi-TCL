#########################################################################
##                                                                     ##
##                  Vulnerability News TCL                             ##
##               by Ryuki <ryuki@allnetwork.or.id                      ##
##                                                                     ##
#########################################################################
...
set feeds(Milw0rm:#ryuki) {30 http://milw0rm.com/rss.php}
set feeds(Securityfocus:#ryuki) {40 http://www.securityfocus.com/rss/vulnerabilities.xml}
set feeds(US-Cert:#ryuki) {50 http://www.kb.cert.org/vuls/atomfeed?OpenView&start=1&count=30}
...
variable version "Vulnerability-News-TCL.v1.0"
	...
	foreach {foo item} [regexp -all -inline {(?i)<item.*?>(.*?)</item>} $data] {
		...
		if {[lsearch -exact $hash($chanfeed) [md5 $title]] == -1 && [botonchan $chan]} {
			...
			if {$count < $maxnew} {
				...
				puthelp "PRIVMSG $chan :0,2 ::8\[11$title8\]0 $descr 8\[11$link8\]0 :: "
				incr count
			} {
				lappend indices $idx
			}
