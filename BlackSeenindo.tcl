...
set dir "/seen/db/${username}.BlackSeen.db"
set dir1 "/seen/db/${username}.seenrecord.db"
...
puthelp "PRIVMSG $chan :Tunggu 1 menit sebelum memulai pencarian."
return 0
}

if {[string match -nocase $what $nick]} { puthelp "PRIVMSG $chan :$nick Ada kaca nggak ? atau kamu habis obat waras ? :)"
return 0
}
if {[onchan $what $chan]} { puthelp "PRIVMSG $chan :$what Sedang berada di $chan :)"
return 0
}
...
set database [read -nonewline $file]
close $file
if {$database == ""} { puthelp "PRIVMSG $chan :Saya tidak memiliki catatan dalam database saya."
return 0
}
...
if {$jointime == "0"} { set staymsg "Saya tidak tahu berapa banyak dia tinggal."
} else { set staymsg "Setelah dia tinggal $staytime di $chan."}
...
puthelp "PRIVMSG $chan :I dont remember $what."
}
}
...
puthelp "PRIVMSG $chan :$bywho mencarin anda di $chan about $howmuch ($date $hour)."
if {$line != ""} {
set num [expr $lin - 1]
set delete [lreplace $datas $num $num]
set files [open $dir1 "w"]
