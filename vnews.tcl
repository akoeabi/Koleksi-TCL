####################################################
## [+] Project	: Advisories Indonesian Coder TCL ##
## [+] Author	: Don Tukulesto 		  ##
## [+] Contact	: root@indonesiancoder.com	  ##
####################################################
...
#Change the channel with your channel name
set feeds(Milw0rm:#jatimcom) {30 http://milw0rm.com/rss.php}
set feeds(Securityfocus:#jatimcom) {40 http://www.securityfocus.com/rss/vulnerabilities.xml}
set feeds(Vupen:#jatimcom) {50 http://www.vupen.com/security-advisories.xml}
set feeds(SecurityReason:#jatimcom) {60 http://securityreason.com/rss/ExploitAlert}
set feeds(PacketStorm:#jatimcom) {70 http://packetstormsecurity.org/exploits.xml}

# maximum number of announced new headlines
variable maxnew 1

# News fetch timeout in seconds
variable timeout 20
...
variable version "Advisories-tcl.v1.0"
...
putlog "$version Indonesian Coder loaded"
...
proc fetch {url chanfeed args} {
	...
	::http::config -useragent "$version Indonesian Coder"
	...
	foreach {foo item} [regexp -all -inline {(?i)<item.*?>(.*?)</item>} $data] {
		...
		if {[lsearch -exact $hash($chanfeed) [md5 $title]] == -1 && [botonchan $chan]} {
			...
			if {$count < $maxnew} {
				...
				puthelp "PRIVMSG $chan :4 ::7\[2$title7\]4 $descr 7\[2$link7\]4 :: "
				incr count
			} {
				lappend indices $idx
			}
...
# Penggabungan Proses base64 dari http://wiki.tcl.tk/775 
proc b64en str {
