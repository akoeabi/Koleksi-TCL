...
# extended by Win a 2 @ irc.quakenete.org #gibt1
# 	now "**" and "!definitions" without quotes tells you all stored definitions via notice
#
...
set learn_whodid ""
bind pub m !definitions learn_definitions 

proc learn_definitions {n u h c a} { 
    global learn_db
    putserv "NOTICE $n :\[ ° ° availalbe definitions ° ° \]"
    set lista ""
    set fp [open $learn_db r]
    while {![eof $fp]} {lappend lista [lindex [split [gets $fp] " "] 1]}
    close $fp
    foreach {a b c d e f g} [lsort -unique $lista] {putserv "NOTICE $n :Learn contains: $a $b $c $d $e $f $g"}
    putserv "NOTICE $n :\[ ° ° === END === ° ° \]" }
...
proc learn_learn { nick uhost hand chan args } {
 ...
 switch [lindex $args 0] {
  ...
  default { puthelp "NOTICE $nick :Syntax: \002!learn <add|del|ins|put|rep>" }
 }
}
...
proc learn_search { nick uhost hand chan args } {
 ...
 if {$args=="" && [isop $nick]} {
    learn_definitions $nick $uhost $hand $chan ""
    return
    } elseif {$args=="" && ![isop $nick]} {
    puthelp "PRIVMSG $chan :\002Sorry, no access.\002"
    return
    }
