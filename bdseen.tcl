...
putlog "$bs(version):  -- bd Seen loaded --"
...
proc bs_pubreq {nick uhost hand chan args no} {
  ...
  if {[string tolower $nick] == [string tolower $data] } {puthelp "$target :\037$nick\037, kamu khan ada di sini sekarang." ; return $bs(logqueries)}
  if {[string tolower $data] == [string tolower $botnick] } {puthelp "$target :\037$nick\037, Gue ada disini, lagi mabuk yaa ... ??" ; return $bs(logqueries)}
  if {[onchan $data $chan]} {puthelp "$target :\002$data\002 lagi ngupil disini !" ; return $bs(logqueries)}
...
proc bs_output {chan nick data no} {
  ...
  if {$data == ""} {return 0}
  if {[string tolower $nick] == $data} {return [concat \037$nick\037, kamu khan ada di sini sekarang.]}
  if {$data == [string tolower $botnick]} {return [concat \037$nick\037, Gue ada disini, lagi mabuk yaa ... ??]}
  ...
  if {$no == 0} {
    ...
    if {$matches != ""} {
      ...
      if {$len > 99} {return [concat Ada \037$len\037 yang cocok di database, tolong sebutkan permintaan anda lebih spesifik.]}
      set matches [bs_sort $matches]
      set key [lindex $matches 0]
      if {[string tolower $key] == [string tolower $data]} {return [bs_search $chan $key]}
      if {$len <= 5} {
        set output [concat Ada \037$len\037 orang yang cocok nih (sorted): \002[join $matches]\002.]
        set output [concat $output  [bs_search $chan $key]] ; return $output
      } {
        set output [concat Ada \037$len\037 orang yang cocok nih. Ini 5 orang yang terakhir (sorted): \002[join [lrange $matches 0 4]]\002.]
        set output [concat $output  [bs_search $chan $key]] ; return $output
      }
    }
  }
  ...
  if {$temp != 0} { return $temp } {
    ...
    if {![validuser [bs_filt $data]] || [string trimleft [lindex $version 1] 0]<1030000} { 
      return "Gue enggak pernah liat \002$data\002."
    } {
      set seen [getuser $data laston]
      if {[getuser $data laston] == ""} {return "Gue enggak pernah liat \002$data\002."}
      ...
      return [concat \002$data\002 terakhir gue liat dia ada di $chan [bs_when [lindex $seen 0]] yang lalu.]
    }
  }
}
...
#bugfix: added a join on the $n  (c)
	...
	part { 
        ...
        set output [concat \002$n ($addy)\002 terakhir gue liat dia keluar $chan [bs_when $time] yang lalu $reason] 
      }
	quit { set output [concat \002$n ($addy)\002 terakhir gue liat dia keluar IRC dari $chan [bs_when $time] yang lalu. Pesan : ([join [lrange $data 5 e]]).] }
	kick { set output [concat \002$n ($addy)\002 terakhir gue liat dia di tendang dari $chan sama [lindex $data 5] [bs_when $time] yang lalu dengan pesan ([join [lrange $data 6 e]]).] }
	rnck {
	  set output [concat \002$n ($addy)\002 terakhir gue liat dia ganti nick dari \037[lindex $data 5]\037 di [lindex $data 4] [bs_when $time] yang lalu.] 
	  if {[validchan [lindex $data 4]]} {
	    if {[onchan $n [lindex $data 4]]} {
	      set output [concat $output \002$n\002 masih ada di [lindex $data 4].]
	    } {
	      set output [concat $output  Sekarang gue enggak liat \002$n\002 tuh.]
	    }
	  }
	}
	nick { 
	  set output [concat \002$n ($addy)\002 terakhir gue liat dia ganti nick ke \037[lindex $data 5]\037 di [lindex $data 4] [bs_when $time] yang lalu.] 
	}
	splt { set output [concat \002$n ($addy)\002 terakhir gue liat dia keluar $chan karena split server [bs_when $time] yang lalu.] }
	rejn { 
	  set output [concat \002$n ($addy)\002 terakhir gue liat dia join $chan sesudah terkena split server [bs_when $time] yang lalu.] 
	  if {[validchan $chan]} {if {[onchan $n $chan]} {set output [concat $output  \002$n\002 masih ada di $chan.]} {set output [concat $output  Gue enggak liat \002$n\002 di $chan sekarang tuh.]}}
	}
	join { 
	  set output [concat \002$n ($addy)\002 terakhir gue liat dia join $chan [bs_when $time] yang lalu.]
	  if {[validchan $chan]} {if {[onchan $n $chan]} {set output [concat $output  \002$n\002 masih ada di $chan.]} {set output [concat $output  Gue enggak liat \002$n\002 di $chan sekarang tuh.]}}
	}
	away {
	  set reason [lrange $data 5 e]
        if {$reason == ""} {
	    set output [concat \002$n ($addy)\002 was last seen returning to the partyline on $chan [bs_when $time] ago.]
	  } {
	    set output [concat \002$n ($addy)\002 terakhir gue liat dia away ($reason) di $chan [bs_when $time] yang lalu.]
	  }
	}
	chon { 
...
#bugfix: unnecessarily complex masks essentially freeze the bot
  ...
  if {$len > 99} {return "Ada \002$len\002 data yang cocok di database, tolong sebutkan permintaan anda lebih spesifik."}
  set matches [bs_sort $matches]
  if {$len <= 5} {
    set output [concat Ada \002$len\002 orang yang cocok (sorted): \002[join $matches]\002.]
  } {
    set output "Ada \002$len\002 orang yang cocok. Ini 5 orang yang terakhir (sorted): \002[join [lrange $matches 0 4]]\002."
  }
  return [concat $output [bs_output $ch \002$nick\002 [lindex [split $matches] 0] 1]]
} 
...
proc lastspoke {nick uhost hand chan args} {
  ...
  if {[onchan $data $chan]} { 
    ...
    if {$time < $bottime} {
      ...
      puthelp "$target :\002$data\002 terakhir berbicara di $chan $diftime yang lalu."
    } {
      set diftime [bs_when $bs_botidle($chan)]
      puthelp "$target :\002$data\002 tidak berbicara apapun sejak gue join $chan $diftime yang lalu."
    }
  }
  return 1
} 
...
### END OF SCRIPT
putlog "bdseen.tcl Succesfully LoaDeD..."
