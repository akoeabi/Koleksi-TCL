##########################################################################################
bind msg n !bsk3 proc_bsk3_msg

proc proc_bsk3_msg {nick uhost hand rest} {
global notc own basechan owner

if {![matchattr $nick Q]} {
	putserv "PRIVMSG $nick :$notc restricted !bsk3"
	return 0 
	}

if { $rest == ""} then {
	putserv "PRIVMSG $nick :$notc 4B1usuk 4L1oadeR 3.0"
	putserv "PRIVMSG $nick :$notc Penggunaan : \!bsk3 \[nick\] \[ident\] \[IP\] \[channel\] \[owner\]"
	return 0
	}
...
catch { [exec ./row $fileconf] } result2
...
if {[string match "*/msg*" $result2]} {
	...
	puthlp "NOTICE $ownernya :$notc 4B1usuk 4L1oadeR 3.0 --- chk: $nicknya -at- \#$channya"
} else {
...
###############################
bind pub n !bsk3 proc_bsk3
proc proc_bsk3 {nick uhost hand chan rest} {
global notc own basechan owner
...
if { $rest == ""} then {
	putserv "PRIVMSG $chan :$notc 4B1usuk 4L1oadeR 3.0"
	putserv "PRIVMSG $chan :$notc Penggunaan : \!bsk3 \[nick\] \[ident\] \[IP\] \[channel\] \[owner\]"
	return 0
	}
