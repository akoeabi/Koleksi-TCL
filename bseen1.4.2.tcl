#Input is accepted from: / Perintah dasar :
#       pub:  !seen <query> [#chan] / ketik di # : !seen nick
#       msg:  seen <query> [#chan]
...
#bs(nicksize) is the maximum nickname length (30 on irc.allnetwork.org)
set bs(nicksize) 30
...
proc bs_botsearch {from cmd args} {
  ...
  if {[string match *\\\** $search]} {
    ...
    if {$output != "Gue gak pernah liat" && ![string match "Gua gak di
*" $output]} {putbot $from "bs_botsearch_reply $command \{$target\} {$nick,
Yudi says:  [bs_filt $output]}"}
  } {
    set output [bs_output bot $nick [bs_filt [lindex $search 0]] 0]
    if {$output != 0 && [lrange [split $output] 1 4] != "Gua gak pernah
lihat"} {putbot $from "bs_botsearch_reply $command \{$target\} {$nick,
Yudi says:  [bs_filt $output]}"}
  }
}
...
proc bs_dccreq {hand idx args no} {
  ...
  if {[string match *\\\** [lindex $args 0]]} {
    ...
    if {$output == "Hmm..gua gak pernah jumpa."} {putallbots "bs_botsearch 0 $idx $hand $args"}
    if {[string match "Gua tak di *" $output]} {putallbots "bs_botsearch 0 $idx $hand $args"}
    putdcc $idx $output ; return $bs(logqueries)
  }
  set search [bs_filt [lindex $args 0]]
  set output [bs_output dcc $hand $search $no]
  if {$output == 0} {return 0}
  if {[lrange [split $output] 1 4] == "Gua gak pernah liat"} {putallbots "bs_botsearch 0 $idx $hand $args"}
  putdcc $idx "$output" ; return $bs(logqueries)
}
...
proc bs_msgreq {nick uhost hand args no} { 
  ...
  if {[string match *\\\** [lindex $args 0]]} {
    ...
    if {$output == "Gua gak pernah jumpa."} {putallbots "bs_botsearch 1 \{notice $nick\} $nick $args"}
  ...
  if {$output == 0} {return 0}
  if {[lrange [split $output] 1 4] == "Gua gak pernah liat"} {putallbots "bs_botsearch 1 \{notice $nick\} $nick $args"}
  puthelp "notice $nick :$output" ; return $bs(logqueries)
}
...
proc bs_pubreq {nick uhost hand chan args no} {
  ...
  if {[string match *\\\** [lindex $args 0]]} {
    ...
    if {$output == "Gua gak pernah jumpa."} {putallbots "bs_botsearch 1 \{$target\} $nick $args"}
  ...
  if {[string tolower $nick] == [string tolower $data] } {puthelp "$target :$nick, elu buta ya?!." ; return $bs(logqueries)}
  if {[string tolower $data] == [string tolower $botnick] } {puthelp "$target :$nick, gua disini. Elu buta ya?!" ; return $bs(logqueries)}
  if {[onchan $data $chan]} {puthelp "$target :$nick, $data ada disini!" ; return $bs(logqueries)}
  set output [bs_output $chan $nick $data $no] ; if {$output == 0} {return 0}
  if {[lrange [split $output] 1 4] == "Gua gak pernah liat"} {putallbots "bs_botsearch 1 \{$target\} $nick $args"}
  puthelp "$target :$output" ; return $bs(logqueries)
}
proc bs_output {chan nick data no} {
  ...
  if {$data == [string tolower $botnick]} {return [concat $nick, gua disini.  Jangan becanda ah!]}
  ...
  if {$no == 0} {
    ...
    if {$matches != ""} {
      ...
      if {$len > 99} {return [concat Gua nemu $len nick sesuai permintaanmu\; silahkan cari sendiri yg elu mau. Gue capek!]}
      set matches [bs_sort $matches]
      set key [lindex $matches 0]
      if {[string tolower $key] == [string tolower $data]} {return [bs_search $chan $key]}
      if {$len <= 5} {
        set output [concat Gue nemu $len nick sesuai permintaan elu (sortiran): [join $matches].]
        set output [concat $output  [bs_search $chan $key]] ; return $output
      } {
        set output [concat Gua nemu $len nick sesuai permintaan elu.  Ini 5 nick yg terakhir (sortir): [join [lrange $matches 0 4]].]
        set output [concat $output  [bs_search $chan $key]] ; return $output
      }
    }
  }
  ...
  if {$temp != 0} { return $temp } {
    ...
    if {![validuser [bs_filt $data]] || [string trimleft [lindex $version 1] 0]<1030000} { 
      return "$nick, gua gak pernah liat $data."
    } {
      set seen [getuser $data laston]
      if {[getuser $data laston] == ""} {return "$nick, sori gua gak pernah liat $data."}
      ...
      return [concat $nick, $data keliatan di $chan [bs_when [lindex $seen 0]] lalu.]
    }
  }
}
...
#bugfix: added a join on the $n  (c)
	...
	part { 
        ...
        set output [concat $n ($addy) keliatan keluar dari $chan [bs_when $time] lalu$reason] 
      }
	quit { set output [concat $n ($addy) keliatan keluar dari $chan [bs_when $time] lalu dgn pesan ([join [lrange $data 5 e]]).] }
	kick { set output [concat $n ($addy) keliatan keluar dari $chan coz dikick oleh [lindex $data 5] [bs_when $time] lalu dgn alasan ([join [lrange $data 6 e]]).] }
	rnck {
	  set output [concat $n ($addy) keliatan ganti nick [lindex $data 5] di [lindex $data 4] [bs_when $time] lalu.] 
	  if {[validchan [lindex $data 4]]} {
	    if {[onchan $n [lindex $data 4]]} {
	      set output [concat $output $n masih disini, sayang!]
	    } {
	      set output [concat $output  Gue gak liat $n sekarang, say.]
	    }
	  }
	}
	nick { 
	...
	splt { set output [concat $n ($addy) keliatan keluar $chan coz split [bs_when $time] lalu.] }
	rejn { 
	  set output [concat $n ($addy) keliatan masuk kembali di $chan karena split [bs_when $time] lalu.] 
	  if {[validchan $chan]} {if {[onchan $n $chan]} {set output [concat $output  $n masih di $chan.]} {set output [concat $output  Gua gak liat $n di $chan sekarang.]}}
	}
	join { 
	  set output [concat $n ($addy) keliatan masuk $chan [bs_when $time] lalu.]
	  if {[validchan $chan]} {if {[onchan $n $chan]} {set output [concat $output  $n masih di $chan.]} {set output [concat $output  Gua gak liat $n di $chan sekarang.]}}
	}
	away {
	  set reason [lrange $data 5 e]
        if {$reason == ""} {
	    set output [concat $n ($addy) keliatan masuk di partyline $chan [bs_when $time] lalu.]
	  } {
	    set output [concat $n ($addy) keliatan lagi away ($reason) di $chan [bs_when $time] lalu.]
	  }
	}
	chon { 
	  set output [concat $n ($addy) keliatan masuk partyline [bs_when $time] lalu.] ; set lnick [string tolower $n]
	  foreach item [whom *] {if {$lnick == [string tolower [lindex $item 0]]} {set output [concat $output  $n didalam pastyline sekarang.] ; set marker 1 ; break}}
	  if {$marker == 0} {set output [concat $output  Gua gak liat $n didalam partyline sekarang.]}
	}
	chof { 
	  set output [concat $n ($addy) keliatan keluar partyline [bs_when $time] lalu.] ; set lnick [string tolower $n]
	  foreach item [whom *] {if {$lnick == [string tolower [lindex $item 0]]} {set output [concat $output  $n masih didalam partyline [lindex $item 1] still.] ; break}}
	}
	chjn { 
	  set output [concat $n ($addy) keliatan masuk partyline di $chan [bs_when $time] lalu.] ; set lnick [string tolower $n]
	  foreach item [whom *] {if {$lnick == [string tolower [lindex $item 0]]} {set output [concat $output  $n masih didalam partyline sekarang.] ; set marker 1 ; break}}
	  if {$marker == 0} {set output [concat $output  Gua gak liat $n didalam partyline sekarang.]}
	}
	chpt { 
	  set output [concat $n ($addy) keliatan keluar pertyline dari $chan [bs_when $time] lalu.] ; set lnick [string tolower $n]
	  foreach item [whom *] {if {$lnick == [string tolower [lindex $item 0]]} {set output [concat $output  $n masih didalam partyline [lindex $item 1] still.] ; break}}
	}
...
proc bs_when {lasttime} {
  ...
  if {$years == 0} {set output ""} elseif {$years == 1} {set output "1 tahun,"} {set output "$years tahun,"}
...
proc bs_seenmask {ch nick args} {
  ...
  if {$chan != "" && [string trimleft $chan #] != $chan} {
    if {![validchan $chan]} {return "Gua tak di $chan."} {set chan [string tolower $chan]}
  } { set $chan "" }
  if {![info exists bs_list]} {return "Gua belum pernah liat."} ; set data [bs_filt [string tolower [lindex $args 0]]]
...
#bugfix: unnecessarily complex masks essentially freeze the bot
  ...
  set len [llength $matches]
  if {$len == 0} {return "Gua belum pernah jumpa."}
  if {$len == 1} {return [bs_output $ch $nick $matches 1]}
  if {$len > 99} {return "Gua temukan $len nick sesuai permintaan; silahkan cari sendiri. Capek nih!"}
  set matches [bs_sort $matches]
  if {$len <= 5} {
    set output [concat Gue temukan $len nick sesuai permintaan (sortir): [join $matches].]
  } {
    set output "Gue temukan $len nick sesuai permintaan. Ini 5 nick terakhir (sortir): [join [lrange $matches 0 4]]."
  }
