#domain whois by #@homer irc.quakenet.org ^^
#a simple domain whois script works fine long time tested
#simple to use: ".dwhois domain.com"
#contact: fkhg1@web.de or #@homer @ irc.quakenet.org
#www.homer-im-bett.de.vu


bind pub - !whois dwhois
...
flush $sockdw
	while {$i <= $l} {
		...
		if {[string match "*Domain*registriert*" $linedw]} {
			gets $sockdw
			putserv "PRIVMSG $chan :owner: [html [gets $sockdw]] street: [html [gets $sockdw]] city: [html [gets $sockdw]] country: [html [gets $sockdw]]"
			close $sockdw
			return 0
		}
		incr i
