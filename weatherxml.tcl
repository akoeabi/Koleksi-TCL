		...
		switch -exact -- $type {
			LOOKUP {
				foreach line [split [::lephttp::data $token] \n] {
					...
					if {[regexp -- {<loc id=\"([^\"]*)\" type=\"([0-9])\">([^<]*)</loc>} $line match id type text]} {
				...
				switch -exact -- [array size curids] {
					...
					1 {
						::lephttp::fetch http://xoap.weather.com/weather/local/[lindex [array get curids] 0]?cc=*&prod=xoap&par=$par&key=$key -command [list ::weather::callback WEATHER $chan $nick] -timeout 60000
					}
					default {
